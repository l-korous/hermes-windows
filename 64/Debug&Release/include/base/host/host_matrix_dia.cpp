...
void HostMatrixDIA<ValueType>::CopyFrom(const BaseMatrix<ValueType> &mat) {
  ...
  if (const HostMatrixDIA<ValueType> *cast_mat = dynamic_cast<const HostMatrixDIA<ValueType>*> (&mat)) {
    ...
    if (this->get_nnz() > 0) {
      ...
      _set_omp_backend_threads(this->local_backend_, this->get_nrow());
...
#pragma omp parallel for
    for (int i=0; i<nrow; ++i) {
      ...
      for (int j=0; j<ndiag; ++j) {
        ...
        if ((i >= start) && (i < end))
          sum += this->mat_.val[DIA_IND(i, j, nrow, ndiag)] * cast_in->vec_[i+v_offset];
        else
          if (i >= end)
            break;
...
#pragma omp parallel for
    for (int i=0; i<nrow; ++i) {
      ...
      for (int j=0; j<ndiag; ++j) {
        ...
        if ((i >= start) && (i < end))
          cast_out->vec_[i] += scalar*this->mat_.val[DIA_IND(i, j, nrow, ndiag)] * cast_in->vec_[i+v_offset];
        else
          if (i >= end)
            break;
