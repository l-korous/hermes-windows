...
template <typename ValueType>
void GPUAcceleratorMatrixCSR<ValueType>::Zeros() {
...
bool GPUAcceleratorMatrixCSR<ValueType>::ExtractInverseDiagonal(BaseVector<ValueType> *vec_inv_diag) const { 
  ...
  if (this->get_nnz() > 0)  {
    ...
    kernel_csr_extract_inv_diag<ValueType, int> <<<GridSize, BlockSize>>> (nrow, this->mat_.row_offset, this->mat_.col, this->mat_.val,
...
bool GPUAcceleratorMatrixCSR<ValueType>::AddScalar(const ValueType alpha) { 
  ...
  if (this->get_nnz() > 0) {
    ...
    kernel_buffer_addscalar<ValueType, int> <<<GridSize, BlockSize>>> (nnz, alpha,
                                                                       this->mat_.val);
    CHECK_CUDA_ERROR(__FILE__, __LINE__);      
...
bool GPUAcceleratorMatrixCSR<ValueType>::DiagonalMatrixMult(const BaseVector<ValueType> &diag) {
  ...
  if (this->get_nnz() > 0) {
    ...
    kernel_csr_diagmatmult<ValueType, int> <<<GridSize, BlockSize>>> (nrow, this->mat_.row_offset, this->mat_.col, 
...
template <typename ValueType>
void GPUAcceleratorMatrixCSR<ValueType>::SymbolicMatMatMult(const BaseMatrix<ValueType> &src) {
  FATAL_ERROR(__FILE__, __LINE__);
}
...
template <typename ValueType>
void GPUAcceleratorMatrixCSR<ValueType>::SymbolicMatMatMult(const BaseMatrix<ValueType> &A, const BaseMatrix<ValueType> &B) {
  FATAL_ERROR(__FILE__, __LINE__);
}
